...
#      'activs': ['elu', 'relu', 'elu', 'relu',  'elu']},
    ...
    {'h_sizes': [16, 64, 32, 32, 32, 16, 8], 
     'activs': ['elu', 'relu', 'elu', 'relu', 'elu', 'relu',  'elu']},
     {'h_sizes': [16, 64, 128, 64, 32, 32, 32, 16, 8], 
...
num_outer_layers = [1,2,3,4,5,7,9] 

learning_rates = [1e-4]#[2e-5, 5e-5, 1e-4, 5e-4, 1e-3]
dropout = 0
l2 = 1e-6
epochs = 500
save_freq = 50
...
inner_optim='learned-gcn'    # vanilla | learned-gcn 

models = [USCA_GCN]      # GCN_ChPt | USCA_GCN 
trainers = [Simple_Trainer_G]

#--- data---

dfn_tr = PROJ_RT+'../../data_my/HataSuburban/channels-HataSuburban-10.h5' # train / validation data
X_tr, y_tr, cinfo_tr = ds.load_data_unsup(dfn_tr)
print(X_tr.shape, y_tr.shape)

dfn_te = PROJ_RT+'../../data_my/HataUrban/channels-HataUrban-10.h5' # test data
...
for learning_rate in learning_rates:
            ...
            for loss_which in loss_options:
                ...
                for MODEL, TRAINER in zip(models, trainers):
                    ...
                    for k, (train_index_, valid_index_) in enumerate(
                        ...
                        save_path = PROJ_RT + 'runs_10_EXP+LR+SplitH/' + fix + f'/{k}/'
    ...
    #                     # instantiate model      
                        model = MODEL(num_layers=num_l, in_size=in_size, out_size=out_size, **arc, channel_info=cinfo_tr,
                                      dropout=dropout, inner_optim=inner_optim).to(device)
